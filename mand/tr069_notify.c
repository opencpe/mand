...
static int notify_is_valid(const struct tr069_element *elem, int ntfy)
{
...
static void reset_notify_table(const struct tr069_table *kw, struct tr069_value_table *st, uint32_t mask);
static void reset_notify_object(const struct tr069_element *elem, struct tr069_instance *base, uint32_t mask);

static void reset_notify_element(const struct tr069_table *kw, struct tr069_value_table *st, int index, uint32_t mask)
{
...
static void tr069_reset_notify_slot(int slot)
{
	uint32_t mask = ~(3 << (slot * 2));

	reset_notify_table(&dm_root, tr069_value_store, mask);
}
		...
		const DM_VALUE value, enum notify_type type)
{
	...
	debug("(): %s, %08x ... %d", tr069_sel2name(sel, b1, sizeof(b1)), ntfy, slot);

	tr069_selcpy(si.sb, sel);
...
DM_RESULT tr069_set_notify_by_selector(const tr069_selector sel, int slot, int value)
{
...
static void set_notify_slot_table(const struct tr069_table *kw, struct tr069_value_table *st, int slot, uint32_t ntfy);
static void set_notify_slot_object(const struct tr069_element *elem, struct tr069_instance *base, int slot, uint32_t ntfy);

static void set_notify_slot_element(const struct tr069_table *kw, struct tr069_value_table *st, int index, int slot, uint32_t ntfy)
{
	const struct tr069_element *elem;
...
DM_RESULT tr069_set_notify_by_selector_recursive(const tr069_selector sel, int slot, int value)
{
	struct tr069_element_ref ref;
...
static void cwmp_notify(void *data __attribute__ ((unused)), struct notify_queue *queue)
{
	...
	RB_FOREACH(item, notify_queue, queue) {
		...
		s = tr069_sel2name(item->sb, buf, sizeof(buf));
