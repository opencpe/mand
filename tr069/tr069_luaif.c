...
static DM_RESULT luaif_tvpair_to_value(lua_State *L, uint32_t type,
				       ...
				       DM_VALUE *value);
static DM_RESULT luaif_set_cb(void *data, const tr069_selector sel,
			      const struct tr069_element *elem,
...
static int luaif_list_cb(void *data, CB_type type, tr069_id id,
			 const struct tr069_element *elem,
...
luaif_tvpair_to_value(lua_State *L, uint32_t type,
		      const struct tr069_element *elem, DM_VALUE *value)
{
	DM_RESULT r = DM_OK;
	...
	if (type == AVP_UNKNOWN) {
		if (!lua_isstring(L, -1))
			...
			debug(": = %s\n", lua_tostring(L, -1));
			r = tr069_string2value(elem, lua_tostring(L, -1), 0, value);
		}
	} else {
		switch (elem->type) {
		case T_STR:
			if (type != AVP_STRING || !lua_isstring(L, -1))
				...
				debug(": = \"%s\"\n", lua_tostring(L, -1));
				r = tr069_set_string_value(value,
							   lua_tostring(L, -1));
			}
		...
		case T_BASE64:
			if (type != AVP_BINARY || !lua_isstring(L, -1))
				...
				r = tr069_set_binary_data(value, len, data);
			}
...
static DM_RESULT
luaif_set_cb(void *data, const tr069_selector sel,
	     const struct tr069_element *elem,
	     struct tr069_value_table *base,
...
LUA_SIG(set)
{
	...
	for (int i = 2; !lua_isnil(L, -1); i++) {
		...
		if (!tr069_name2sel(lua_tostring(L, -2), &sel)) {
		...
		if ((rc = tr069_get_value_ref_by_selector_cb(sel, &sel /* ...tweak... */,
...
static DM_RESULT
luaif_get_cb(void *data, const tr069_selector sb __attribute__((unused)),
	     const struct tr069_element *elem, const DM_VALUE val)
{
	...
	case T_ENUM:
		...
		case AVP_UNKNOWN:
			type = AVP_ENUM;
		case AVP_ENUM:
			lua_pushstring(L, tr069_int2enum(&elem->u.e,
						   	 DM_ENUM(val)));
...
LUA_SIG(get)
{
	...
	GET_BY_SELECTOR_CB get_value = getCfgSessionStatus() == CFGSESSION_ACTIVE_LUAIF ?
					tr069_cache_get_value_by_selector_cb :
					tr069_get_value_by_selector_cb;
	...
	for (int i = 2; !lua_isnil(L, -1); i++) {
		...
		if (!tr069_name2sel(lua_tostring(L, -1), &sel)) {
			lua_pushinteger(L, DM_VALUE_NOT_FOUND);
			return 1;
		}
...
LUA_SIG(retrieve_enums)
{
	...
	if ((rc = tr069_get_value_by_selector_cb(sel, T_ENUM, L,
 ...
 */
static int
luaif_list_cb(void *data, CB_type type, tr069_id id,
	      const struct tr069_element *elem,
	      const DM_VALUE value __attribute__((unused)))
{
	lua_State	*L = data;
	...
	if (lua_isnil(L, -1)) {		/* hack that prevents the first element from being processed */
		lua_pop(L, 1);		/* later tr069_walk_by_name might be modified or reimplemented */
...
LUA_SIG(list)
{
	...
	if (!tr069_walk_by_selector_cb(sel, 2, L, luaif_list_cb)) {
		lua_pushinteger(L, DM_ERROR);
		return 1;
	}
...
LUA_SIG(add)
{
	...
	id = TR069_ID_USER_OBJECT;
	if (!tr069_add_instance_by_selector(sel, &id)) {
		lua_pushinteger(L, DM_ERROR);
		return 1;
	}
...
LUA_SIG(delete)
{
	...
	lua_pushinteger(L, tr069_del_table_by_selector(sel) ? DM_OK : DM_ERROR);
	return 1;
}
...
LUA_SIG(find)
{
	...
	if (!(kw = tr069_get_object_table_by_selector(sel))) {
		lua_pushinteger(L, DM_INVALID_TYPE);
		return 1;
	}

	if ((paramId = tr069_get_element_id_by_name(param, strlen(param), kw)) == TR069_ERR) {
		lua_pushinteger(L, DM_VALUE_NOT_FOUND);
		return 1;
	}
	...
	tr069_free_any_value(kw->table + paramId - 1, &value);
	if (!inst) {
		lua_pushinteger(L, DM_ERROR);
		return 1;
	}
...
LUA_SIG(deserialize_file)
{
	...
	lua_pushinteger(L, tr069_deserialize_file(file, flags) ? DM_ERROR : DM_OK);
	return 1;
}
...
LUA_SIG(deserialize_directory)
{
	...
	lua_pushinteger(L, tr069_deserialize_directory(dir, flags) ? DM_ERROR : DM_OK);
	return 1;
}
...
init_Lua_environment(void)
{
	...
	lua_register_type_constants(L);		/* FIXME: maybe use types from tr069_token.h */
...
#if 0
DM_VALUE
get_Lua_function(tr069_selector sel)
{
	DM_VALUE val;
...
int
set_Lua_function(tr069_selector sel, DM_VALUE val)
{
	return 0;
}
#endif
