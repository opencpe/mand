...
typedef struct tr069_instance_node ENTRY;
...
typedef struct tr069_instance_tree {
	STRUCT_MAGIC_START
	const struct index_definition *definition;

	struct tr069_value_table *cntr_base;
	tr069_id cntr_id;
 ...
 *      }
 * node_ptr ->
 *      struct tr069_instance_node;
 *      struct tr069_value_table;
 *
 * Note: instance_node and struct tr069_value_table are variable size structures!
 */
...
TREE *tr069_alloc_instance(const struct tr069_element *kw, struct tr069_instance *inst)
{
...
void tr069_free_instance(struct tr069_instance *inst)
{
	dm_assert(inst != NULL);
...
void tr069_instance_set_counter(struct tr069_instance *inst, struct tr069_value_table *cntr_base, tr069_id cntr_id)
{
	dm_assert(inst != NULL);
	dm_assert(inst->instance != NULL);
	assert_struct_magic(inst->instance, INSTANCE_MAGIC);
...
static int cmp_entry(TREE *head, int idx, ENTRY *a, ENTRY *b)
{
	...
	if (type == T_INSTANCE) {
		...
		return tr069_compare_values(type,
...
static int cmp_value(TREE *head, int idx, DM_VALUE *val, ENTRY *b)
{
	...
	if (type == T_INSTANCE) {
		...
		return tr069_compare_values(type, val, &DM_TABLE(b->table)->values[element - 1]);
}
...
static int id2idx(const struct index_definition *def, tr069_id id)
{
...
struct tr069_instance_node *tr069_instance_root(struct tr069_instance *inst)
{
	const int idx = 0;
...
struct tr069_instance_node *tr069_instance_first(struct tr069_instance *inst)
{
	dm_assert(inst != NULL);
	assert_struct_magic(inst->instance, INSTANCE_MAGIC);
...
struct tr069_instance_node *tr069_instance_last(struct tr069_instance *inst)
{
	dm_assert(inst != NULL);
	assert_struct_magic(inst->instance, INSTANCE_MAGIC);
...
struct tr069_instance_node *tr069_instance_next(struct tr069_instance *inst,
...
struct tr069_instance_node *tr069_instance_prev(struct tr069_instance *inst,
						struct tr069_instance_node *node)
{
	dm_assert(inst != NULL);
	dm_assert(inst->instance != NULL);
...
struct tr069_instance_node *tr069_instance_first_idx(struct tr069_instance *inst, tr069_id id)
{
	dm_assert(inst != NULL);
	assert_struct_magic(inst->instance, INSTANCE_MAGIC);
...
struct tr069_instance_node *tr069_instance_last_idx(struct tr069_instance *inst, tr069_id id)
{
	dm_assert(inst != NULL);
	assert_struct_magic(inst->instance, INSTANCE_MAGIC);
...
struct tr069_instance_node *tr069_instance_next_idx(struct tr069_instance *inst, tr069_id id,
						    struct tr069_instance_node *node)
{
	dm_assert(inst != NULL);
	dm_assert(inst->instance != NULL);
...
struct tr069_instance_node *tr069_instance_prev_idx(struct tr069_instance *inst, tr069_id id,
						    struct tr069_instance_node *node)
{
	dm_assert(inst != NULL);
	dm_assert(inst->instance != NULL);
...
tr069_id tr069_idm2id(struct tr069_instance *inst, int idm)
{
	if (idm) {
		for (struct tr069_instance_node *node = tr069_instance_first(inst);
		     node != NULL;
		     node = tr069_instance_next(inst, node))
			if (node->idm == idm)
...
unsigned int tr069_instance_node_count(struct tr069_instance *inst)
{
	dm_assert(inst != NULL);
	dm_assert(inst->instance != NULL);
	assert_struct_magic(inst->instance, INSTANCE_MAGIC);
...
static void init_indexes(struct tr069_instance_node *row, int size)
{
...
static void clear_indexes(struct tr069_instance_node *row, int size)
{
	struct node *node = ROW2NODE(row) - size;
	struct index_nodes *idx = ((struct index_nodes *)node) - 1;
...
void insert_instance(struct tr069_instance *inst, struct tr069_instance_node *row)
{
#if defined(SDEBUG)
	char b1[128];
#endif
	...
	if (inst->instance->cntr_base)
		tr069_incr_counter_by_id(inst->instance->cntr_base, inst->instance->cntr_id);
}

void remove_instance(struct tr069_instance *inst, struct tr069_instance_node *row)
{
	dm_assert(inst != NULL);
	dm_assert(inst->instance != NULL);
	assert_struct_magic(inst->instance, INSTANCE_MAGIC);
	...
	if (inst->instance->cntr_base)
		tr069_decr_counter_by_id(inst->instance->cntr_base, inst->instance->cntr_id);
}

static void update_idx(struct tr069_instance_tree *tree, int idx, struct tr069_instance_node *row)
{
#if defined(SDEBUG)
	char b1[128];
#endif
...
void update_index(tr069_id id, struct tr069_instance_node *row)
{
	ENTER();

	debug(": id: %d, node: %p", id, row);

	struct tr069_instance_tree *tree = row->root;
	if (!tree)
		return;
...
void update_instance_node_index(struct tr069_instance_node *row)
{
	struct tr069_instance_tree *tree = row->root;
	if (!tree)
		return;
...
struct tr069_instance_node *find_instance(struct tr069_instance *inst, tr069_id id, int type, DM_VALUE *val)
{
	ENTER();
	...
	if (idx < 0) {
		...
		for (row = MINMAX(inst->instance, 0, RB_NEGINF);
		     ...
		     row = TREE_NEXT(0, row)) {
			assert_index_magic(row, INDEX_MAGIC);
			if (tr069_compare_values(type, val, &DM_TABLE(row->table)->values[id - 1]) == 0)
				return row;
		}
...
struct tr069_instance_node *tr069_alloc_instance_node(const struct tr069_table *kw, const tr069_selector base, tr069_id id)
{
	ENTER();
	...
	int size =
		...
		idx_size +
		sizeof(ENTRY) +
		sizeof(struct tr069_value_table) + sizeof(DM_VALUE) * kw->size;
	...
	node = (struct tr069_instance_node *)(((struct node *)(idx + 1)) + kw->index->size);
	node->instance = id;
	init_struct_magic(node, NODE_MAGIC);
	assert_index_magic(node, INDEX_FREE_MAGIC);      /* structure pointer sanity check */

	tr069_init_table(kw, (struct tr069_value_table *)(node + 1), base, id);
	set_DM_TABLE(node->table, (struct tr069_value_table *)(node + 1));
...
void tr069_free_instance_node(const struct tr069_table *kw, struct tr069_instance_node *row)
{
...
extern const struct tr069_table keyword_388_tab;
...
int main(void)
{
	...
	tr069_alloc_instance(&inst, &def_388, keyword_388_tab.size);

	for (int i = 1; i < 100; i++) {
		char *s;

		node = tr069_alloc_instance_node(&keyword_388_tab);
		if (!node)
			break;

		node->instance = i;
		asprintf(&s, "Node %d", i);
		tr069_set_string_by_id(DM_TABLE(node->table), cwmp__IGD_LANDev_i_Hosts_H_j_HostName, s);
		tr069_set_string_by_id(DM_TABLE(node->table), cwmp__IGD_LANDev_i_Hosts_H_j_MACAddress, s);
