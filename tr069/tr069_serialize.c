			      ...
			      const struct tr069_element *elem, const DM_VALUE value)
{
	...
	*/
	...
	if (value.notify & 0x0003)
		snprintf(nfbuf, sizeof(nfbuf), " notify=\"%s\"", tr069_int2enum(&notify_attr, value.notify & 0x0003));
...
*/
		...
		case T_ENUM:
			findent(stream, indent); fprintf(stream, "<%s%s>%s</%s>\n",
							 elem->key, nfbuf, tr069_int2enum(&elem->u.e, DM_ENUM(value)), elem->key);
			break;
		case T_STR:
			findent(stream, indent);
		...
		case T_SELECTOR: {
			...
			if (DM_SELECTOR(value))
				s = tr069_sel2name(*DM_SELECTOR(value), buf, sizeof(buf));
			if (s) {
...
static int walk_cb(void *userData, CB_type type, tr069_id id,
		   const struct tr069_element *elem, const DM_VALUE value)
{
	int r = 1;
	...
	switch (type) {
		case CB_object_instance_start:
			debug(": %d", id);
			if (((id & TR069_ID_AUTO_OBJECT) != TR069_ID_AUTO_OBJECT &&
...
{
	struct walk_data w;
	tr069_selector sel = { cwmp__InternetGatewayDevice, 0 };
	...
	tr069_walk_by_selector_cb(sel, TR069_SELECTOR_LEN, &w, walk_cb);
}
