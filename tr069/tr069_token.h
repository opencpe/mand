#ifndef __TR069_TOKEN_H
#define __TR069_TOKEN_H
...
#define TR069_SELECTOR_LEN    16
typedef uint16_t tr069_id;
typedef tr069_id tr069_selector[TR069_SELECTOR_LEN];
#define TR069_ERR UINT16_MAX

struct tr069_value_table;
struct tr069_instance_tree;
struct tr069_instance_node;

struct tr069_instance {
	struct tr069_instance_tree *instance;
};
...
typedef struct {
	union {
		...
		char                     *string;
		binary_t                 *binary;
		tr069_selector           *selector;
		struct tr069_value_table *table;
		struct tr069_instance    instance;
...
struct tr069_instance_node {
	...
	struct tr069_instance_tree *root;
	STRUCT_MAGIC_END
};
...
#define cast_table2node(t) (((struct tr069_instance_node *)(t)) - 1)
#define cast_node_table_ref2node(t) generic_struct_cast(struct tr069_instance_node, table, t)

struct tr069_token;
struct tr069_element;
struct tr069_table;

struct tr069_value_fkts {
	int (*validate)(const struct tr069_value_table *, tr069_id, const struct tr069_element *, DM_VALUE, unsigned int *, char **);
	DM_VALUE (*get)(const struct tr069_value_table *, tr069_id, const struct tr069_element *, DM_VALUE);
	int (*set)(struct tr069_value_table *,tr069_id, const struct tr069_element *, DM_VALUE *, DM_VALUE);
};

struct tr069_instance_fkts {
	void (*add)(const struct tr069_table *, tr069_id, struct tr069_instance *, struct tr069_instance_node *);
	void (*del)(const struct tr069_table *, tr069_id, struct tr069_instance *, struct tr069_instance_node *);
};

struct tr069_int_limits {
	int min;
	int max;
};
...
struct tr069_element {
	...
	union {
		const struct tr069_value_fkts value;
		const struct tr069_instance_fkts instance;
	} fkts;
	union {
		struct {
			const struct tr069_table *table;
			unsigned int max;
		} t;
		const struct tr069_int_limits l;
		const struct tr069_enum e;
		tr069_id counter_ref;
	} u;
};
...
struct tr069_table {
	...
	int size;
	struct tr069_element table[];
};

extern const struct tr069_table dm_root;
extern struct tr069_value_table *tr069_value_store;
...
static inline size_t tr069_sellen(const tr069_selector sel) __attribute__((nonnull (1)));
static inline void tr069_selcpy(tr069_selector s1, const tr069_selector s2) __attribute__((nonnull (1,2)));
static inline void tr069_selcat(tr069_selector sel, tr069_id id) __attribute__((nonnull (1)));

size_t tr069_sellen(const tr069_selector sel)
{
	int i;

	for (i = 0; i < TR069_SELECTOR_LEN && sel[i]; i++)
		;

	return i;
}

void tr069_selcpy(tr069_selector s1, const tr069_selector s2)
{
	memcpy(s1, s2, sizeof(tr069_selector));
}

void tr069_selcat(tr069_selector sel, tr069_id id)
{
	for (int i = 0; i < TR069_SELECTOR_LEN; i++) {
		if (sel[i] == 0) {
			sel[i] = id;
			if (i + 1 < TR069_SELECTOR_LEN)
				sel[i + 1] = 0;
			break;
		}
	}
}
#endif /* __TR069_TABLE_H */
