...
static tr069_id next_token(void *userData, const struct tr069_table *kw, int type);
...
DM_RESULT
tr069_string2value(const struct tr069_element *elem, const char *str, uint8_t set_update, DM_VALUE *value)
{
	uint8_t		updated;
	DM_RESULT	res = DM_OK;
	...
	switch (elem->type) {
		case T_STR:
			...
			res = tr069_set_string_value(value, str);

			break;

		case T_BINARY:
			/* FIXME: this is not entirely correct */
			res = tr069_set_binary_data(value, strlen(str), str);
			break;
		...
		case T_BASE64: {
			...
			updated = tr069_binarycmp(DM_BINARY(*value), n) != 0;
			res = tr069_set_binary_value(value, n);
			free(n);
		...
		case T_SELECTOR: {
			...
			if (*str) {
				if (!tr069_name2sel(str, &sb)) {
					res = DM_INVALID_VALUE;
					break;
				}
			} else
				memset(&sb, 0, sizeof(tr069_selector));

			updated = DM_SELECTOR(*value) ?
					tr069_selcmp(*DM_SELECTOR(*value), sb, TR069_SELECTOR_LEN) != 0 : 1;

			res = tr069_set_selector_value(value, sb);
		...
		case T_ENUM: {
			...
			if ((i = tr069_enum2int(&elem->u.e, str)) == -1)
				res = DM_INVALID_VALUE;
			else {
...
tr069_selector *
tr069_name2sel(const char *name, tr069_selector *sel)
{
	int i;
	int st_type;
	const char *s = name;
	...
	for (i = 0; i < TR069_SELECTOR_LEN && kw; i++) {

		tr069_id id = next_token(&s, kw, st_type);
		if (id == 0) {
			debug("(): no more elements\n");
			break;
		} else if (id == TR069_ERR) {
	   ...
	   int type)
{
	...
	if (type == T_OBJECT) {
		...
		return tr069_get_element_id_by_name(token, i, kw);
}
