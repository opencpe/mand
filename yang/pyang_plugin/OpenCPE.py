...
class TreePlugin(plugin.PyangPlugin):
    ...
    def emit(self, ctx, modules, fd):
        ...
        for element in top_elements:
            ...
            fd.write(3*tab + ".key    = \"" + make_key(element) + "\",\n")
            fd.write(3*tab + ".flags  = F_VERSION,\n")
...
builtin_types = ['binary', 'bits', 'boolean', 'decimal64', 'empty', 'enumeration', 
                  'identityref', 'instance-identifier', 'int8', 'int16', 'int32', 
                    'int64', 'leafref', 'string', 'uint8', 'uint16', 'uint32', 'uint64', 'union',
                        'inet:ipv4-address', 'inet:ipv6-address']
            ...
            if child.keyword in ['container', 'list', 'leaf', 'leaf-list'] and get_xpath(child) not in deviations.keys():
                ...
                for groupchild in grouping.substmts:
                    if groupchild.keyword in ['container', 'list', 'leaf', 'leaf-list', 'choice']:
                        counter += print_field(fd, groupchild, typedefs, annotations, counter, prefix=make_name(s)+'_')
...
def print_field(fd, child, typedefs, annotations, counter, prefix=''):
    ...
    if child.keyword in ['leaf'] or counter == -1:
        ...
        union_flag = False
        if type.arg == 'union':
            types = type.search('type')
            field_counter = len(child.search_one('type').search('type'))
            union_flag = True
        ...
        for i in range(field_counter):
            ...
            if union_flag:
                key = make_key(type_i, child.arg + '_')
            elif child.keyword == 'leaf-list':
                key = "list_" + make_key(child)
            else:
                key = make_key(child)

            name = make_name(child.parent)
            header_collector.insert(0, "#define " + "field_" + name + "_" + key + " " + str(counter) + "\n")

            fd.write(2*tab + "{\n") 
            fd.write(3*tab + "/* " + str(counter) + " */\n") 
            fd.write(3*tab + ".key = " + "\"" + key + "\"" + ",\n") 
            ...
            if type.arg == 'enumeration':
                print_type(fd, type, "field_" + name + "_" + key + "_")
            else: 
                print_type(fd, type)
        ...
        fd.write(3*tab + ".key = " + "\"" + make_key(child) + "\"" + ",\n") 
        fd.write(3*tab + ".flags = " + "F_READ | F_WRITE" + ",\n") 
        if action == "":
            fd.write(3*tab + ".action = DM_NONE" + ",\n") 
        else:
...
def make_key(s, prefix=''):
    with_hyphens = prefix + s.arg
    key = ''
